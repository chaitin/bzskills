...
This file provides guidance to AI coding agents working on the `bzskills` CLI codebase.

## Project Overview

`bzskills` is the CLI for the open agent skills ecosystem.

## Commands

| Command                       | Description                                         |
| ----------------------------- | --------------------------------------------------- |
| `bzskills`                      | Show banner with available commands                 |
| `bzskills add <pkg>`            | Install skills from git repos, URLs, or local paths |
| `bzskills experimental_install` | Restore skills from skills-lock.json                |
| `bzskills experimental_sync`    | Sync skills from node_modules into agent dirs       |
| `bzskills list`                 | List installed skills (alias: `ls`)                 |
...
### How `bzskills check` and `bzskills update` Work
...
6. `bzskills update` reinstalls changed skills by invoking the current CLI entrypoint directly (`node <repo>/bin/cli.mjs add <source-tree-url> -g -y`) to avoid nested npm exec/npx behavior
...
| -------------------------- | ------------------------------------------------------------- |
| `bzskills add`               | `src/add.ts` - full implementation                              |
| `bzskills experimental_sync` | `src/sync.ts` - crawl node_modules                              |
| `bzskills check`             | `src/cli.ts` + `fetchSkillFolderHash` in `src/skill-lock.ts`    |
| `bzskills update`            | `src/cli.ts` direct hash compare + reinstall via `bzskills add` |
